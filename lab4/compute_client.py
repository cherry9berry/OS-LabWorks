...
class ComputeClient:
    def __init__(self):
        self.pipe = None
        self.root = tk.Tk()
        self.root.title("Compute Client")
        ...
        # Событие ввода: начали/закончили печатать
    ...
    def register(self):
        write_message(self.pipe, "REGISTER:ComputeProcessor:Compute")
        resp = read_message(self.pipe)
        assert resp.startswith("NAME:"), "registration failed"
        _, self.client_name, _ = resp.split(':', 2)
        write_message(self.pipe, "Started:Compute client started")

    def on_send(self):
        txt = self.text_var.get()
        if not txt:
            return
        # Локальная обработка: если число — отправляем квадрат, иначе исходное
        try:
            val = float(txt)
            out = str(val * val)
        except ValueError:
            out = txt
        write_message(self.pipe, f"COMPUTING:{out}")
        self.text_var.set("")

    # Отправка статусов начала/окончания ввода
    def on_text_change(self, *args):
        txt = self.text_var.get()
        if len(txt) > 0 and not self.is_typing:
            write_message(self.pipe, "Computing:Начало ввода")
            self.is_typing = True
        elif len(txt) == 0 and self.is_typing:
            write_message(self.pipe, "Computing:Конец ввода")
            self.is_typing = False
    ...
    def on_close(self):
        try:
            write_message(self.pipe, "Stopped:Compute client stopped")
        except Exception:
            pass
        self.root.destroy()
